...
async function checkPair(args, exchange, datetime) {
  ...
  if (inputTokenDecimals == 18 && outputTokenDecimals == 18) {
    ...
    (inputTokenSymbol == "USDC" && outputTokenSymbol == "USDT") ||
    (outputTokenSymbol == "USDC" && inputTokenSymbol == "USDT")
  ) {
    // output USDC and USDT Have 6 Decimals
    pairReserves0 = BigInt(
      Number(pairReserves._reserve0) * 1000000000000
    ).toString();
    pairReserves1 = BigInt(
      Number(pairReserves._reserve1) * 1000000000000
    ).toString();
    ...
    pairReserves0 = BigInt(
      Number(pairReserves._reserve0) * 10000000000
    ).toString();
    pairReserves1 = pairReserves._reserve1;
  } else if (outputTokenSymbol == "WBTC") {
    pairReserves0 = pairReserves._reserve0;
    pairReserves1 = BigInt(
      Number(pairReserves._reserve1) * 10000000000
    ).toString();
  }

  if (
    (inputTokenSymbol == "WETH" && outputTokenSymbol != "USDT") ||
    (inputTokenSymbol == "ETH" && outputTokenSymbol != "USDT")
  ) {
  ...
  const getPairRateO = await dexRouterContract.methods
    .getAmountOut(inputAmount, pairReserves1, pairReserves0)
    .call();
  const getPairRate1 = await dexRouterContract.methods
    .getAmountOut(inputAmount, pairReserves0, pairReserves1)
    .call();

  let sql = `
    -- insert getgPairRateO
    INSERT INTO RATES (exchange_id, pair_id, direction, rate, datetime) VALUES (${dexId}, ${pairId}, '${directionOne}', ${web3.utils.fromWei(
    getPairRateO,
    "Ether"
  )}, '${datetime}');

    -- insert getPairRate1
    INSERT INTO RATES (exchange_id, pair_id, direction, rate, datetime) VALUES (${dexId}, ${pairId}, '${directionTwo}', ${web3.utils.fromWei(
    getPairRate1,
    "Ether"
  )}, '${datetime}');
  `;
